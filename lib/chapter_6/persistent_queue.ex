defmodule PersistentQueue do
  ...
  Initializes a queue
  """
  def init(), do: %PersistentQueue{front: Suspension.create(:empty), rear: Suspension.create(:empty)}

  @doc """
  Adds an element to the queue
  """
  def snoc(%PersistentQueue{} = queue, el) do
    check(%PersistentQueue{queue | length_r: queue.length_r + 1, rear: Suspension.create(%Cons{head: el, tail: queue.rear})})
  end

  defp check(%PersistentQueue{length_f: lenf, length_r: lenr} = q) when lenr <= lenf, do: q
  defp check(queue) do
    %PersistentQueue{length_f: queue.length_f + queue.length_r,
